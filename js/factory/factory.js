...
const factory = function ($http, $q) {
  ...
  const find = function () {
    const def = $q.defer()
    const url = baseUrl + 'public/characters?limit=' + limit + '&apikey=' + publicKey + '&hash=bd4b447a65ef5d6b174f87cf9db6d2db&ts=1'
  ...
  const findOne = function (id) {
    const def = $q.defer()
    const url = baseUrl + 'public/characters/' + id + '?apikey=' + publicKey + '&hash=bd4b447a65ef5d6b174f87cf9db6d2db&ts=1'
    $http.get(url).success(def.resolve).error(def.reject)
  ...
  const findNext = function (offset) {
    const def = $q.defer()
    const url = baseUrl + 'public/characters?limit=' + limit + '&offset=' + (limit * offset) + '&apikey=' + publicKey + '&hash=bd4b447a65ef5d6b174f87cf9db6d2db&ts=1'
    $http.get(url).success(def.resolve).error(def.reject)
